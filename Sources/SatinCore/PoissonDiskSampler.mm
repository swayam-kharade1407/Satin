...
Points2D generatePoissonDiskSamples(Rectangle rect, float minDistance, float kMax) {
    ...
    const simd_float2 initialSample = bounds.min + simd_make_float2
    (
    ...
    const simd_int2 initialSampleGridPosition = gridSizeHalf + simd_make_int2
    (
        initialSample.x/cellSize,
        initialSample.y/cellSize
    );
    ...
    while (!activeList.empty()) {
        ...
        for(int k = 0; k < kMax; k++) {
            ...
            if(rectangleContainsPoint(bounds, candidate)) {
                const simd_int2 candidateGridPosition = gridSizeHalf + simd_make_int2
                (
