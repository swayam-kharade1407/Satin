...
fragment float4 gridFragment( CustomVertexData in [[stage_in]] )
{
    ...
    float sdfXY = unionHard( sdfX, sdfY ) * dot( normal, float3( 0, 0, 1 ) );
    sdfXY = 1.0 - saturate( sdfXY / fwidth( sdfXY ) );

    float sdfYZ = unionHard( sdfY, sdfZ ) * dot( normal, float3( 1, 0, 0 ) );
    sdfYZ = 1.0 - saturate( sdfYZ / fwidth( sdfYZ ) );

    float sdfXZ = unionHard( sdfX, sdfZ ) * dot( normal, float3( 0, 1, 0 ) );
    sdfXZ = 1.0 - saturate( sdfXZ / fwidth( sdfXZ ) );

    float sdf = min( min( sdfXY, sdfYZ ), sdfXZ );
    return float4( float3(sdf), 1.0 );
}
