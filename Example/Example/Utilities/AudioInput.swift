...
class AudioInput: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    func stopCapture() {
        if captureSession.isRunning, let captureInput = captureInput {
    ...
    func setupCapture() {
        ...
        var newInputCaptureDevice: AVCaptureDeviceInput?
        do {
        ...
        if let captureInput = captureInput, captureSession.inputs.contains(captureInput) {
            captureSession.removeInput(captureInput)
        }
    ...
    @objc public func captureOutput(_: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
    ...
    func getInputDevice(_: String) -> AVCaptureDevice? {
        let devices = getInputDevices()
