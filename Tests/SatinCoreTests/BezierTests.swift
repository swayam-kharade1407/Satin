...
class BezierTests: XCTestCase {
                                   ...
                                   _ pts: inout [SIMD2<Float>])
    {
        ...
        if (startMiddleAngle + middleEndAngle) > angleLimit {
            ...
            self._adaptiveQuadBezierCurve2(a, ab, abc, aVel, sVel, bVel, angleLimit,
                                           depth + 1, &pts)
            pts.append(abc)

            let eVel = simd_normalize(quadraticBezierVelocity2(abc, bc, c, 0.5))
            self._adaptiveQuadBezierCurve2(abc, bc, c, bVel, eVel, cVel, angleLimit, depth + 1, &pts)
        }
    }
                                    ...
                                    _ angleLimit: Float) -> [SIMD2<Float>]
    {
        ...
        self._adaptiveQuadBezierCurve2(a, b, c, aVel, bVel, cVel, angleLimit, 0, &pts)
    ...
    func testAdaptiveQuadBezierPath2Perf() {
        self.measure {
            for _ in 0..<100 {
                let polyline = self.getAdaptiveQuadBezierPath2(.init(0, 0), .init(1, 0), .init(0, 1), 0.001)
                XCTAssertEqual(polyline.count, 513)
            }
        }
    }
