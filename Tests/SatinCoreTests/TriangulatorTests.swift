...
class TriangulatorTests: XCTestCase {
    ...
    var overlappingPath: [simd_float2] = [
        [-1, -1],
        [-1, -1],
        [-1, -1],
        [1, -1],
    ...
    func testTriangulateOverlappingPoints() {
        var triData = createTriangleData()

        let result = triangulatePath(&overlappingPath, Int32(overlappingPath.count), &triData)
        XCTAssertEqual(result, 0)

        for i in 0..<Int(triData.count) {
            print(triData.indices[i])
        }
